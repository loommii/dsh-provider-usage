  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css="' + styleId + '"]') === null) {
      var css = [
        ...
        ".zGbnIq_modelTable{width:100%;border-collapse:collapse;font-size:12px;line-height:18px}.zGbnIq_modelTable th,.zGbnIq_modelTable td{text-align:left;padding:5px 8px;vertical-align:middle;white-space:nowrap}.zGbnIq_modelTable thead th{color:var(--dsw-alias-label-tertiary);font-weight:500;border-bottom:1px solid var(--dsw-alias-border-l2)}.zGbnIq_modelTable tbody tr{border-bottom:1px solid var(--dsw-alias-border-l2)}.zGbnIq_modelTable tbody tr:last-child{border-bottom:none}.zGbnIq_modelTable tbody tr:hover{background:var(--dsw-alias-interactive-bg-hover)}.zGbnIq_modelTable .zGbnIq_num{text-align:right;font-variant-numeric:tabular-nums;overflow:visible}.zGbnIq_modelTable .zGbnIq_numText{display:inline-block;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;vertical-align:bottom}.zGbnIq_modelTable th.zGbnIq_candidateId{max-width:280px;overflow:visible;font-family:var(--ds-font-family-code);font-weight:400;font-size:13px}.zGbnIq_modelTable .zGbnIq_modelName{position:relative;display:inline-block;max-width:280px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;vertical-align:bottom;cursor:help}.zGbnIq_modelTable .zGbnIq_modelName:hover .zGbnIq_modelNameTooltip,.zGbnIq_modelTable .zGbnIq_modelName:focus .zGbnIq_modelNameTooltip{opacity:1;visibility:visible}.zGbnIq_modelNameTooltip{position:absolute;left:0;top:calc(100% + 6px);z-index:30;background:var(--dsw-alias-bg-module-platform);color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:6px;padding:2px 6px;font-size:12px;line-height:18px;white-space:nowrap;opacity:0;visibility:hidden;transition:opacity .12s,visibility .12s;box-shadow:0 4px 12px rgba(0,0,0,.18);pointer-events:none}.dsh-pu-numCell{position:relative;cursor:help;display:inline-block;max-width:100%;vertical-align:bottom}.dsh-pu-numCell:hover .dsh-pu-numTooltip,.dsh-pu-numCell:focus .dsh-pu-numTooltip{opacity:1;visibility:visible}.dsh-pu-numTooltip{position:absolute;right:0;top:calc(100% + 6px);z-index:30;background:var(--dsw-alias-bg-module-platform);color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:6px;padding:2px 6px;font-size:12px;line-height:18px;white-space:nowrap;opacity:0;visibility:hidden;transition:opacity .12s,visibility .12s;box-shadow:0 4px 12px rgba(0,0,0,.18);pointer-events:none}",
    ...
    // 缩略数字单元格：点击在「缩写 ↔ 完整数字」之间切换（完整值不撑破列宽，<table> 整列同步变宽）；
    // hover 浮层 tooltip 作为补充（不点击也能瞄一眼）；aria-label 供读屏读出完整值。
    function NumCell(props) {
      ...
      return React.createElement('span', {
        className: props.className + ' dsh-pu-numCell',
        role: 'button',
        tabIndex: 0,
        ...
        full ? long : short,
        React.createElement('span', { className: 'dsh-pu-numTooltip', role: 'tooltip' }, long));
    }
    ...
    // 按模型汇总表列宽（auto 布局下作为期望宽度提示；数字列内容自适应，长值不会截断）
    var MODEL_COL_WIDTHS = ['34%', '12%', '16%', '18%', '12%', '8%'];
    ...
    function UsageStatsPage(props) {
      ...
      // ── 按模型汇总表（语义化 <table>）──
      // 最佳实践：表格语义交给 <table>；列宽 auto 布局 + 内容自适应（数字列永不截断）；
      // 长模型名单行省略 + hover 浮层；完整数字走 NumCell 浮层 tooltip，不参与布局 → 无 layout shift。
      // 模型名：单行省略，hover 浮层显示完整名（长模型名不折行、不撑破列）
      function ModelNameCell(props) {
        var name = props.name;
        return React.createElement('span', {
          className: 'zGbnIq_modelName',
          tabIndex: 0,
          title: name,
        }, name,
          React.createElement('span', { className: 'zGbnIq_modelNameTooltip', role: 'tooltip' }, name));
      }
      function ModelSummaryTable(props) {
        var rows = props.rows;
        return React.createElement('table', { className: 'zGbnIq_modelTable' },
          React.createElement('colgroup', null,
            MODEL_COL_WIDTHS.map(function (w, i) { return React.createElement('col', { key: i, style: { width: w } }); })),
          React.createElement('thead', null,
            React.createElement('tr', null,
              React.createElement('th', { scope: 'col' }, '模型'),
              React.createElement('th', { scope: 'col', className: 'zGbnIq_num' }, '请求'),
              React.createElement('th', { scope: 'col', className: 'zGbnIq_num' }, '输入'),
              React.createElement('th', { scope: 'col', className: 'zGbnIq_num' }, '输出'),
              React.createElement('th', { scope: 'col', className: 'zGbnIq_num' }, '缓存读取'),
              React.createElement('th', { scope: 'col', className: 'zGbnIq_num' }, '命中率'))),
          React.createElement('tbody', null,
            rows.map(function (r) {
              return React.createElement('tr', { key: r.name },
                React.createElement('th', { scope: 'row', className: 'zGbnIq_candidateId' }, React.createElement(ModelNameCell, { name: r.name })),
                React.createElement('td', { className: 'zGbnIq_num' }, React.createElement('span', { className: 'zGbnIq_numText' }, String(r.requests))),
                React.createElement('td', { className: 'zGbnIq_num' }, React.createElement(NumCell, { value: r.inputTokens })),
                React.createElement('td', { className: 'zGbnIq_num' }, React.createElement(NumCell, { value: r.outputTokens })),
                React.createElement('td', { className: 'zGbnIq_num' }, React.createElement(NumCell, { value: r.cacheReadTokens })),
                React.createElement('td', { className: 'zGbnIq_num' }, React.createElement('span', { className: 'zGbnIq_numText' }, r.hitRate)));
            })));
      }
