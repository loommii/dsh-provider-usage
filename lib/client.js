...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const DEFAULT_SETTINGS = { visible: true, size: 160, refreshMs: 30000, posEpoch: 0, settingsVersion: 3 };
    // z-index 固定为与 dsh-pet 相同的值（2147483000，≈int32 上限），不向用户开放设置
    ...
    const zh = {
      ...
      nav: '用量中心',
      settingsTitle: '用量中心',
      settingsDesc: '供应商用量/余额/套餐查询（当前支持 OpenCode Go，后续扩展多厂家）',
      secDefaults: '恢复默认设置',
      statusDesc: '数据源：opencode.ai/zen/go/v1/usage（OpenCode Go）',
      secVisible: '显示悬浮鲸鱼',
      secSize: '鲸鱼尺寸（px）',
      secRefresh: '面板自动刷新间隔',
      secPos: '位置',
    ...
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css="' + styleId + '"]') === null) {
      var css = [
        ...
        '.dsh-pu-dialog{position:fixed;width:330px;max-height:70vh;overflow:auto;border-radius:14px;pointer-events:auto;box-sizing:border-box;background:var(--dsw-alias-bg-overlay,#1c2333);border:1px solid var(--dsw-alias-border-l1,rgba(255,255,255,.12));box-shadow:0 12px 36px rgba(0,0,0,.35);padding:14px 16px;font-family:inherit;color:var(--dsw-alias-label-primary,#e6e9f2);font-size:13px;line-height:1.5}',
        ...
        // settings page
        // ── 设置页：dsh-pet 同款卡片样式（参照 shared settings-card.module.css 的 token/字形）──
        '.dsh-pu-s{max-width:680px;display:flex;flex-direction:column;gap:12px;font-family:inherit}',
        '.dsh-pu-s-card{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:12px;transition:border-color .16s, background .16s}',
        '.dsh-pu-s-card:hover{border-color:var(--dsw-alias-label-dimmed)}',
        '.dsh-pu-s-head{width:100%;border-radius:12px;align-items:center;gap:12px;padding:14px 16px;display:flex}',
        '.dsh-pu-s-headText{flex-direction:column;flex:1;gap:4px;min-width:0;display:flex}',
        '.dsh-pu-s-name{color:var(--dsw-alias-label-primary);font-size:15px;font-weight:600;line-height:1.4}',
        '.dsh-pu-s-desc{color:var(--dsw-alias-label-tertiary);font-size:13px;line-height:1.5}',
        '.dsh-pu-s-body{border-top:1px solid var(--dsw-alias-border-l2);margin:0 16px;padding-bottom:8px}',
        '.dsh-pu-s-field{flex-direction:column;gap:6px;padding:12px 0;display:flex}',
        '.dsh-pu-s-field+.dsh-pu-s-field{border-top:1px solid var(--dsw-alias-border-l2)}',
        '.dsh-pu-s-headRow{align-items:center;gap:8px;display:flex}',
        '.dsh-pu-s-label{min-width:0;color:var(--dsw-alias-label-primary);flex:1;font-size:13px;font-weight:500;line-height:1.5}',
        '.dsh-pu-s-hint{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:1.5}',
        '.dsh-pu-s-input{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);height:34px;font:inherit;color:var(--dsw-alias-label-primary);border-radius:8px;padding:0 12px;font-size:13px;line-height:1.5}',
        '.dsh-pu-s-input:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}',
        '.dsh-pu-s-check{accent-color:var(--dsw-alias-brand-primary);width:16px;height:16px}',
        '.dsh-pu-s-btn{border:1px solid var(--dsw-alias-border-l2);color:var(--dsw-alias-label-secondary);background:transparent;border-radius:8px;padding:5px 14px;font-size:13px;line-height:1.5;cursor:pointer}',
        '.dsh-pu-s-btn:hover{border-color:var(--dsw-alias-label-dimmed);color:var(--dsw-alias-label-primary)}',
        '.dsh-pu-s-btn:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}',
        '.dsh-pu-s-reset{font:inherit;color:var(--dsw-alias-label-secondary);cursor:pointer;background:transparent;border:none;padding:0;font-size:12px;line-height:1.5}',
        '.dsh-pu-s-reset:hover{color:var(--dsw-alias-label-primary)}',
        '.dsh-pu-s-status{display:flex;flex-direction:column;gap:8px}',
        '.dsh-pu-s-statusLine{display:flex;align-items:center;gap:8px;font-size:12px;line-height:1.5;color:var(--dsw-alias-label-secondary)}',
      ].join('');
    ...
    function WhaleBadge() {
      ...
      // 动态层级：固定 Z_INDEX（同 dsh-pet）；面板打开时 🐳 降到遮罩/面板之下
      var z = Z_INDEX;
      ...
      useLayoutEffect(function () {
        ...
        var top = above ? Math.max(8, pos.top - 12 - h) : pos.top + s.size + 10;
        // 实测宽度钳制：右边缘最多到 vw-8，绝不越界
        var left = Math.max(8, Math.min(pos.left + s.size / 2 - w / 2, window.innerWidth - w - 8));
    ...
    function StatusCard() {
      ...
      return React.createElement('div', { className: 'dsh-pu-s-status' },
        React.createElement('div', { className: 'dsh-pu-s-statusLine' },
          React.createElement('span', { className: 'dsh-pu-dot ' + dot }), ' ' + line),
        React.createElement('div', { className: 'dsh-pu-s-statusLine' },
          zh.source + '：opencode.ai/zen/go/v1/usage' + (cfg ? '（baseUrl ' + cfg.baseUrl + '，超时 ' + cfg.timeoutMs + 'ms）' : '')),
        data && data.credential && data.credential.source ? React.createElement('div', { className: 'dsh-pu-s-statusLine' },
          zh.keyFrom + '：' + data.credential.source + (data.credential.keyHint ? ' (' + data.credential.keyHint + ')' : '')) : null,
        React.createElement('div', { className: 'dsh-pu-s-statusLine' },
          React.createElement('button', { type: 'button', className: 'dsh-pu-s-btn', onClick: load }, zh.refresh)));
    }
    ...
    function SettingsCard() {
      ...
      // dsh-pet 同款卡片：标题卡 + 字段卡 + 状态卡
      return React.createElement('div', { className: 'dsh-pu-s' },
        React.createElement('div', { className: 'dsh-pu-s-card' },
          React.createElement('div', { className: 'dsh-pu-s-head' },
            React.createElement('div', { className: 'dsh-pu-s-headText' },
              React.createElement('span', { className: 'dsh-pu-s-name' }, zh.settingsTitle),
              React.createElement('span', { className: 'dsh-pu-s-desc' }, zh.settingsDesc))),
          React.createElement('div', { className: 'dsh-pu-s-body' },
            React.createElement('div', { className: 'dsh-pu-s-field' },
              React.createElement('div', { className: 'dsh-pu-s-headRow' },
                React.createElement('span', { className: 'dsh-pu-s-label' }, zh.secVisible),
                React.createElement('input', {
                  type: 'checkbox', className: 'dsh-pu-s-check', checked: !!s.visible,
                  onChange: function (e) { setSettings({ visible: e.target.checked }); },
                }))),
            React.createElement('div', { className: 'dsh-pu-s-field' },
              React.createElement('div', { className: 'dsh-pu-s-headRow' },
                React.createElement('span', { className: 'dsh-pu-s-label' }, zh.secSize),
                React.createElement('input', {
                  type: 'number', style: { width: 110 }, className: 'dsh-pu-s-input', value: s.size, min: SIZE_MIN, max: SIZE_MAX, step: 4,
                  onChange: function (e) { setNum('size', e.target.value, SIZE_MIN, SIZE_MAX); },
                })),
              React.createElement('span', { className: 'dsh-pu-s-hint' }, SIZE_MIN + '–' + SIZE_MAX + 'px，默认 160（与 dsh-pet 一致）')),
            React.createElement('div', { className: 'dsh-pu-s-field' },
              React.createElement('div', { className: 'dsh-pu-s-headRow' },
                React.createElement('span', { className: 'dsh-pu-s-label' }, zh.secRefresh),
                React.createElement('select', {
                  className: 'dsh-pu-s-input', value: s.refreshMs,
            ...
            React.createElement('div', { className: 'dsh-pu-s-field' },
              React.createElement('div', { className: 'dsh-pu-s-headRow' },
                React.createElement('span', { className: 'dsh-pu-s-label' }, zh.secPos),
                React.createElement('button', { type: 'button', className: 'dsh-pu-s-btn', onClick: resetPos }, zh.btnResetPos)),
              React.createElement('span', { className: 'dsh-pu-s-hint' }, '默认 right:24 / bottom:20（与 dsh-pet 一致）')),
            React.createElement('div', { className: 'dsh-pu-s-field' },
              React.createElement('div', { className: 'dsh-pu-s-headRow' },
                React.createElement('span', { className: 'dsh-pu-s-label' }, zh.secDefaults),
                React.createElement('button', { type: 'button', className: 'dsh-pu-s-reset', onClick: resetAll }, zh.btnDefaults))))),
        React.createElement('div', { className: 'dsh-pu-s-card' },
          React.createElement('div', { className: 'dsh-pu-s-head' },
            React.createElement('div', { className: 'dsh-pu-s-headText' },
              React.createElement('span', { className: 'dsh-pu-s-name' }, zh.statusTitle),
              React.createElement('span', { className: 'dsh-pu-s-desc' }, zh.statusDesc))),
          React.createElement('div', { className: 'dsh-pu-s-body' },
            React.createElement(StatusCard, null))),
        React.createElement('div', { className: 'dsh-pu-s-hint' }, zh.hintInstant));
    }

    // ── 挂载：🐳 走 body 顶层 React root（与 dsh-pet 同构），设置页走插槽 ──
    function apply(ctx) {
      var rootEl = null;
      var reactRoot = null;

      // 与 dsh-pet 完全一致的挂载方式：document.body 上自建 root。
      // 这样 🐳 与 better-sidebar / pet 同处 body 顶层（z-index: 2147483000），
      ...
      function mount() {
        ...
        reactRoot.render(React.createElement(WhaleBadge));
      }

      function ensureMounted() {
        try {
          if (!getSettings().visible) return; // 用户主动隐藏，不自动复活
          if (rootEl && rootEl.isConnected && document.querySelector('.dsh-pu-badge')) return;
          mount();
        } catch (e) {
