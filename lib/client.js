...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function UsageStatsPage(props) {
      ...
      var [range, setRange] = useState('0'); // 0=全部 7=近7天 30=近30天
      ...
      var modelRows = modelNames.map(function (mn) {
        ...
        return React.createElement("div", { key: mn, className: "zGbnIq_modelRow", style: { gridTemplateColumns: "minmax(0,1.2fr) 44px 52px 52px 52px 52px" } },
        ...
        total ? React.createElement("div", null,
          ...
          React.createElement("div", { className: "zGbnIq_modelCatalog" },
            React.createElement("div", { className: "zGbnIq_modelCatalogHeading" },
              ...
              React.createElement("p", { className: "zGbnIq_modelCatalogMeta" }, "请求 / 输入 / 输出 / 缓存读取 / 命中率")),
      ...
      var providersPanel = React.createElement('div', { className: 'zGbnIq_section' },
          ...
          React.createElement('ul', { className: 'zGbnIq_rows' },
            providers.length === 0
              ...
              var configured = pl.ref ? !!refStatus[pl.ref] : false;
              var dotCls = '';
              var dotLabel = null;
              if (pl.ref) {
                var refKnown = Object.prototype.hasOwnProperty.call(refStatus, pl.ref);
                if (!refKnown && refCheckFailed) { dotCls = ' zGbnIq_credentialDotMissing'; dotLabel = '状态检查失败'; }
                else if (!refKnown) { dotCls = ' dsh-pu-dot-loading'; dotLabel = '检查 Key…'; }
                else if (!configured) { dotCls = ' dsh-pu-dot-gray'; dotLabel = 'Key 未配置'; }
              ...
              var isFinal = !!qs && !qs.loading; // 有最终结果（成功或失败）才可点击展开
              var isErr = dotCls.indexOf('zGbnIq_credentialDotMissing') > -1;
              var clickable = isFinal || (pl.ref && !refKnownRow && refCheckFailed);
              var detailEl = null;
              if (errOpen[pl.id]) {
                if (pl.ref && !refKnownRow && refCheckFailed) {
                  detailEl = React.createElement('p', { className: 'zGbnIq_error', style: { margin: '10px 0 0' } }, '状态检查失败：无法读取 Key 配置状态。');
                } else if (qs && !qs.ok) {
                  detailEl = React.createElement('p', { className: 'zGbnIq_error', style: { margin: '10px 0 0' } }, '查询失败：' + (qs.err || '未知错误'));
                } else if (qs && qs.ok) {
                  detailEl = React.createElement('div', { className: 'zGbnIq_json' }, typeof qs.resp === 'string' ? qs.resp : JSON.stringify(qs.resp || {}, null, 2));
                }
              }
                ...
                React.createElement('div', { className: 'zGbnIq_rowHead' },
                  React.createElement('span', { className: 'zGbnIq_rowIdentity' },
                    ...
                    dotCls ? React.createElement('span', {
                      ...
                      role: 'img', 'aria-label': dotLabel,
                      style: clickable ? { cursor: 'pointer' } : undefined,
                      onClick: clickable ? function () {
                        setErrOpen(function (prev) { var next = Object.assign({}, prev); next[pl.id] = !prev[pl.id]; return next; });
                      } : undefined,
                    }) : null),
                  React.createElement('span', { className: 'zGbnIq_rowActions' },
                    React.createElement('button', { type: 'button', className: 'zGbnIq_secondaryButton', onClick: function () { startEdit(pl); } }, '编辑'),
                    React.createElement('button', { type: 'button', className: 'zGbnIq_dangerButton', onClick: function () { removeProvider(pl); } }, '删除'))),
                detailEl);
            })),
