  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css="' + styleId + '"]') === null) {
      var css = [
        ...
        '.dsh-pu-reset{font-size:10px;color:#C9B8BE;text-align:right;white-space:nowrap;font-variant-numeric:tabular-nums}',
    ...
    function WindowRow(props) {
      // Rainytoken 小组件同款行：标签 | 已用% | 进度条（窗口固定色）| 重置时间
    ...
    function UsageCard() {
      ...
      var load = useCallback(function () {
        if (!current) return;
        setLoading(true);
        var qs = 'adapter=' + encodeURIComponent(current.adapter) + (current.ref ? '&ref=' + encodeURIComponent(current.ref) : '') + (current.source === 'vault' ? '&source=vault' : '');
      ...
      // Rainytoken 状态语义：正常/注意（stale 或余额不足）/错误
      var level = 'err';
      var chipText = '错误';
      if (data && data.ok && data.isValid !== false) { level = 'ok'; chipText = '正常'; }
      else if (data && data.stale) { level = 'warn'; chipText = '未更新'; }
      else if (data && data.ok && data.isValid === false) { level = 'warn'; chipText = '注意'; }
      ...
      return React.createElement('div', { ref: cardRef, className: 'dsh-pu-card',
        ...
        React.createElement('div', { className: 'dsh-pu-card-head' },
          ...
          React.createElement('span', { className: 'dsh-pu-chip ' + level }, React.createElement('i', null), chipText)),
        menu,
        body,
        React.createElement('div', { className: 'dsh-pu-card-foot' },
          ...
          React.createElement('button', { type: 'button', className: 'dsh-pu-refresh', title: zh.refresh, onClick: function () { if (draggedRef.current) { draggedRef.current = false; return; } load(); } }, '↻')));
    }
    ...
    function SettingsCard() {
      ...
      useEffect(function () {
        ...
        if (refs.length === 0) { setRefStatus({}); setQueryStatus({}); return undefined; }
          ...
          providers.forEach(function (p) {
            ...
            jobs.push(fetch('/api/provider-usage/query?' + qs, { headers: { accept: 'application/json' } })
              ...
              .then(function (res) {
                ...
                setQueryStatus(function (prev) {
                  var next = Object.assign({}, prev);
                  next[key] = { ok: !!(res && res.ok), err: (res && res.error && res.error.message) || '' };
                  return next;
                });
              })
              .catch(function () {
        ...
        fetch('/api/provider-usage/credential-refs?refs=' + encodeURIComponent(refs.join(',')), { headers: { accept: 'application/json' } })
          ...
          .catch(function () {
            if (alive) startQueries(null, nonce > 0);
          });
      ...
      return React.createElement('div', { className: 'zGbnIq_section' },
          ...
          React.createElement('ul', { className: 'zGbnIq_rows' },
            providers.length === 0
              ...
              : providers.map(function (p) {
              var qs = p.ref ? queryStatus[p.adapter + '|' + p.ref] : null;
              var configured = p.ref ? !!refStatus[p.ref] : false;
              var dotCls = '';
              var dotLabel = null;
              if (p.ref) {
                if (!configured) { dotCls = ' dsh-pu-dot-gray'; dotLabel = 'Key 未配置'; }
                else if (!qs || qs.loading) { dotCls = ' dsh-pu-dot-loading'; dotLabel = '查询中…'; }
                else if (qs.ok) { dotCls = ' zGbnIq_credentialDotConfigured'; dotLabel = '查询正常'; }
                else { dotCls = ' zGbnIq_credentialDotMissing'; dotLabel = '查询失败' + (qs.err ? '：' + qs.err : ''); }
              }
              return React.createElement('li', { key: p.id, className: 'zGbnIq_rowCard' },
                React.createElement('div', { className: 'zGbnIq_rowHead' },
                  React.createElement('span', { className: 'zGbnIq_rowIdentity' },
                    React.createElement('span', { className: 'zGbnIq_rowName' }, p.name),
                    dotCls ? React.createElement('span', {
                      className: 'zGbnIq_credentialDot' + dotCls,
                      role: 'img', 'aria-label': dotLabel, title: dotLabel,
                    }) : null),
                  React.createElement('span', { className: 'zGbnIq_rowActions' },
                    React.createElement('button', { type: 'button', className: 'zGbnIq_secondaryButton', onClick: function () { startEdit(p); } }, '编辑'),
                    React.createElement('button', { type: 'button', className: 'zGbnIq_dangerButton', onClick: function () { removeProvider(p); } }, '删除'))));
            })),
