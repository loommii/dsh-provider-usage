...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const ADAPTER_META = {
      ...
      'commandcode-credits': { displayName: 'Command Code（订阅+余额）' },
    };
    ...
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css="' + styleId + '"]') === null) {
      var css = [
        ...
        '.dsh-pu-cc-logo{display:flex;align-items:center;justify-content:center;width:30px;height:30px;border-radius:8px;background:linear-gradient(135deg,#5B3A9E,#8E5BD8);color:#fff;font-size:13px;font-weight:800;letter-spacing:.5px;font-family:inherit;flex:none}',
        '.dsh-pu-cc-logo-sm{width:22px;height:22px;border-radius:6px;font-size:10px}',
        '.dsh-pu-cc-sub{display:flex;align-items:center;gap:10px;margin:-6px 0 6px;font-size:11px;color:#C9B8BE;white-space:nowrap}',
        '.dsh-pu-cc-sub .dsh-pu-cc-used{color:#EFE0E5;font-weight:600}',
        '.dsh-pu-cc-sub .dsh-pu-cc-remain{color:#FFB3C6;font-weight:600}',
    ...
    function WindowRow(props) {
      ...
      // 兼容读：resetsAt 为统一命名（parseUsage 与 Command Code 均输出）；resetAt 兜底旧缓存载荷
      var resetsAt = win.resetsAt || win.resetAt;
    ...
    function UsageCard() {
      ...
      var isCommandCode = current.adapter === 'commandcode-credits';
      var logoClass = isBalance ? 'deepseek' : (isCommandCode ? 'commandcode' : 'go');
      var logoEl = isBalance
        ? React.createElement('img', { src: LOGO_DEEPSEEK, width: 24, height: 24, alt: '' })
        : (isCommandCode
          ? React.createElement('span', { className: 'dsh-pu-cc-logo', 'aria-hidden': 'true' }, 'CC')
          : LOGO_OCGO);
      ...
      if (loading && !data) {
        ...
        var winRows = data.windows
          ? (isCommandCode
            ? [React.createElement(WindowRow, { key: 'r', label: zh.rolling, win: data.windows.fiveHour, bar: 'pink' }),
               React.createElement(WindowRow, { key: 'w', label: zh.weekly, win: data.windows.weekly, bar: 'orange' }),
               // 订阅失败/未知计划 → 月% 推算不出（null）：隐藏整行，避免「0% 还没用」的误导
               (data.monthly && data.monthly.usedPct !== null
                 ? React.createElement(WindowRow, { key: 'm', label: zh.monthly, win: data.monthly, bar: 'magenta' })
                 : null)].filter(Boolean)
        ...
        if (isCommandCode) {
          ...
          cc = React.createElement('div', { className: 'dsh-pu-cc-sub' },
            React.createElement('span', { className: 'dsh-pu-cc-used' }, '已用 $' + usedStr + ' / $' + quotaStr),
            React.createElement('span', { className: 'dsh-pu-cc-remain' }, '剩余 $' + remainStr));
        }
        ...
        menu = React.createElement('div', { ref: menuRef, className: 'dsh-pu-menu' },
          providers.map(function (p) {
            ...
            var mini = p.adapter === 'balance-json'
              ...
              : (p.adapter === 'commandcode-credits'
                ? React.createElement('span', { className: 'dsh-pu-cc-logo dsh-pu-cc-logo-sm', 'aria-hidden': 'true' }, 'CC')
                : LOGO_OCGO);
    ...
    function SettingsCard() {
      ...
      function openManual() {
        ...
        setEdit(null);
        setFormError(null);
        setForm({ provider: '', name: 'Command Code', adapter: 'commandcode-credits', key: '', nameTouched: false });
      }
            ...
            React.createElement('select', {
              className: 'zGbnIq_input zGbnIq_selectInput', value: form.adapter,
                onChange: function (e) {
                  ...
                  if (!form.nameTouched) next.name = e.target.value === 'balance-json' ? 'DeepSeek' : (e.target.value === 'commandcode-credits' ? 'Command Code' : 'OpenCode Go');
                  setForm(next);
                },
            }, adapterOptions)),
          React.createElement('div', { className: 'zGbnIq_field' },
