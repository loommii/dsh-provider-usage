...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const DEFAULT_SETTINGS = { visible: true, zIndex: 2147483000, size: 56, refreshMs: 30000, posEpoch: 0 };
    ...
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css="' + styleId + '"]') === null) {
      var css = [
        '.dsh-pu-badge{position:fixed;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:grab;user-select:none;touch-action:none;pointer-events:auto;background:linear-gradient(135deg,#3b7cff,#2563eb);border:2px solid rgba(255,255,255,.25);box-shadow:0 6px 20px rgba(37,99,235,.35);transition:transform .15s ease}',
    ...
    function loadPos(size) {
      ...
      return clampPos(window.innerWidth - size - 24, window.innerHeight - size - 120, size);
    }
    ...
    function WhaleBadge() {
      ...
      if (pos) {
        ...
        var top = pos.top >= 420 ? Math.max(8, pos.top - s.size - 16 - 340) : pos.top + s.size + 10;
      ...
      return React.createElement(React.Fragment, null,
        React.createElement('div', {
          ...
          style: pos ? { left: pos.left, top: pos.top, width: s.size, height: s.size, fontSize: Math.round(s.size * 0.5), zIndex: open ? z - 3 : z } : null,
        ...
        open ? React.createElement('div', { className: 'dsh-pu-dialog', style: Object.assign({ zIndex: z - 1 }, dialogStyle), onClick: function (e) { e.stopPropagation(); } },
    ...
    function SettingsCard() {
      ...
      return React.createElement('div', { className: 'dsh-pus' },
        ...
        React.createElement('div', { className: 'dsh-pus-row' },
          ...
          React.createElement('input', {
            type: 'number', className: 'dsh-pus-input', value: s.size, min: 32, max: 160, step: 4,
            onChange: function (e) { setNum('size', e.target.value, 32, 160); },
          })),
        React.createElement('div', { className: 'dsh-pus-row' },
