...
//       /api/provider-usage/providers（提供方实例清单持久化，v0.8.0 —— host 端 providers.json 为单一事实源）。
...
import { stat as fsStat, readFile, writeFile, rename, chmod, mkdir } from 'node:fs/promises'
...
export function apply(ctx, rawConfig) {
  const config = {
    ...
    // 提供方实例清单（v0.8.0）：host 端单一事实源（localStorage 只是缓存），防重启后清单被静默重建
    providersFile: join(resolveProviderUsageDir(), 'providers.json'),
  }
  ...
  // ── 提供方实例清单持久化（v0.8.0）─
  // 修复「重启后提供方清单被静默重建」：实例清单此前只存浏览器 localStorage，
  // 存储 key 丢失（换 origin/清站点数据/浏览器驱逐）后 loadProviders() 走 DEFAULT_PROVIDERS
  // 兜底 → 已删除的默认实例复活、自定义实例（如 Command Code）永久消失。
  // 现在 host 端 providers.json 为单一事实源；客户端启动时从 host 恢复，写操作同步落盘。
  const PROVIDER_ADAPTER_IDS = new Set(ADAPTERS.map((a) => a.id))
  const PROVIDERS_MAX = 64

  /** 规范并校验一个提供方实例（不合法字段一律剔除，而非整单拒绝——避免脏数据把恢复流程卡死）。 */
  function normalizeProviderEntry(raw) {
    if (!raw || typeof raw !== 'object') return null
    if (typeof raw.adapter !== 'string' || !PROVIDER_ADAPTER_IDS.has(raw.adapter)) return null
    const id = typeof raw.id === 'string' && raw.id.trim() !== '' ? raw.id.trim().slice(0, 128) : null
    if (!id) return null
    const name = typeof raw.name === 'string' && raw.name.trim() !== '' ? raw.name.trim().slice(0, 64) : ''
    const ref = typeof raw.ref === 'string' ? raw.ref.slice(0, 128) : ''
    const source = raw.source === 'vault' ? 'vault' : 'dsh'
    const type = raw.type === 'manual' || raw.type === 'import' ? raw.type : (source === 'vault' ? 'manual' : 'import')
    return { id, name, adapter: raw.adapter, ref, source, type, paused: raw.paused === true }
  }

  /** 读清单：providers.json 缺失/损坏 → null（调用方决定兜底，损坏文件同时改名留证）。 */
  async function readProvidersDoc() {
    let raw
    try {
      raw = await readFile(config.providersFile, 'utf8')
    } catch (e) {
      if (e && e.code === 'ENOENT') return null
      return null // 其它读错误按缺失处理
    }
    try {
      const doc = JSON.parse(raw)
      if (!doc || doc.version !== 1 || !Array.isArray(doc.providers)) throw new Error('bad doc')
      const providers = doc.providers.map(normalizeProviderEntry).filter(Boolean)
      return { version: 1, providers }
    } catch (e) {
      // 损坏文件改名留证（.corrupt-<ts>），下次写盘重建；绝不静默吞掉用户清单
      try { await rename(config.providersFile, config.providersFile + '.corrupt-' + Date.now()) } catch { /* ignore */ }
      return null
    }
  }

  /** 写清单：原子写（tmp → rename）+ 0600（与 credentials.json 同款边界）；目录缺失时先建（0700）。 */
  async function writeProvidersDoc(providers) {
    const doc = { version: 1, providers, savedAt: Date.now() }
    await mkdir(dirname(config.providersFile), { recursive: true, mode: 0o700 }).catch(() => {})
    const tmp = config.providersFile + '.tmp'
    await writeFile(tmp, JSON.stringify(doc, null, 2), { mode: 0o600 })
    await chmod(tmp, 0o600)
    try { await rename(tmp, config.providersFile) } catch (e) { await writeFile(config.providersFile, JSON.stringify(doc, null, 2), { mode: 0o600 }) }
    return doc
  }
  ...
  async function doLocalUsage(target, since, key, state) {
    ...
    // 组装响应（chips/totals/byModel 均按选中提供方过滤；v0.7.0 起 byModel 按提供方嵌套可过滤）
  ...
  ctx.effect(() => {
    const disposers = [
      ...
      // 提供方实例清单持久化（v0.8.0）─：host 端 providers.json 为单一事实源。
      // GET 读清单（缺失返回 ok:true + items:null，由客户端走一次性迁移）；
      // POST 整单覆盖保存（幂等）。仅回环，条目逐个规范化，不合法条目剔除。
      ctx.webServer.register({
        kind: 'exact',
        path: '/api/provider-usage/providers',
        handler: jsonRoute(async (req, body) => {
          if (req.method === 'POST') {
            if (!body || !Array.isArray(body.providers)) {
              return { ok: false, error: { type: 'bad-providers', message: 'providers 必须是数组', httpStatus: 400 } }
            }
            if (body.providers.length > PROVIDERS_MAX) {
              return { ok: false, error: { type: 'too-many-providers', message: '提供方实例最多 ' + PROVIDERS_MAX + ' 个', httpStatus: 400 } }
            }
            const providers = body.providers.map(normalizeProviderEntry).filter(Boolean)
            await writeProvidersDoc(providers)
            return { ok: true, providers }
          }
          const doc = await readProvidersDoc()
          return { ok: true, providers: doc ? doc.providers : null, exists: doc !== null }
        }, true),
      }),
    ]
