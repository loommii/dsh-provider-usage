...
// 资产路由：/api/provider-usage/asset/whale-refined/*（鲸鱼娘精致版图集，仅回环）
...
const WHALE_ASSETS = {
  'pet.json': { mime: 'application/json; charset=utf-8', file: 'pet.json' },
  'spritesheet.webp': { mime: 'image/webp', file: 'spritesheet.webp' },
}
...
export function apply(ctx, rawConfig) {
  ...
  // 资产处理器：只允许白名单内的两个文件（无路径拼接，杜绝穿越）
  const assetHandler = (asset) => async (req, res) => {
    ...
    try {
      ...
      const url = new URL('../assets/whale-refined/' + asset.file, import.meta.url)
  ...
  ctx.effect(() => {
    const disposers = [
      ...
      ctx.webServer.register({
        kind: 'exact',
        path: '/api/provider-usage/asset/whale-refined/pet.json',
        handler: assetHandler(WHALE_ASSETS['pet.json']),
      }),
      ctx.webServer.register({
        kind: 'exact',
        path: '/api/provider-usage/asset/whale-refined/spritesheet.webp',
        handler: assetHandler(WHALE_ASSETS['spritesheet.webp']),
      }),
    ]
