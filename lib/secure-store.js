...
export function createSecureStore(dir) {
  ...
  async function init() {
    ...
    await writeJson(credFile, await readJson(credFile))
  }

  async function set(ref, value) {
    const doc = await readJson(credFile)
    doc.credentials[ref] = encrypt(key, value)
    await writeJson(credFile, doc)
  }

  async function get(ref) {
    const doc = await readJson(credFile)
    const token = doc.credentials[ref]
  ...
  async function has(ref) {
    const doc = await readJson(credFile)
    return Object.prototype.hasOwnProperty.call(doc.credentials, ref)
  }

  async function list() {
    const doc = await readJson(credFile)
    return Object.keys(doc.credentials)
  }

  async function remove(ref) {
    const doc = await readJson(credFile)
    if (Object.prototype.hasOwnProperty.call(doc.credentials, ref)) {
      delete doc.credentials[ref]
      await writeJson(credFile, doc)
    }
  }
